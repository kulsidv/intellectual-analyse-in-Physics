...
def do(mode):
    ...
    model = NeuroNet().fit(X_train, y_train, epoches=350)
    y_pred = model.predict(scaler_X.transform(X_test))
    y_pred_orig = scaler_y.inverse_transform(y_pred)
    print(f"mean_absolute_error: {mean_absolute_error(y_test, y_pred_orig)}")
    print(f"mean_squared_error: {mean_squared_error(y_test, y_pred_orig)}")
